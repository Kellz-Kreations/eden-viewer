...
function Get-PlexHeaders {
  ...
  return @{
    ...
    'X-Plex-Product'          = 'eden-viewer'
    'X-Plex-Version'          = '1'
    'X-Plex-Device'           = 'eden-viewer-setup'
    'X-Plex-Platform'         = 'Windows'
    'Accept'                  = 'application/json'
  }
}

function Get-PlexClaimToken {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory=$true)][string]$ClientIdentifier,
  ...
  $headers = Get-PlexHeaders -ClientIdentifier $ClientIdentifier

  Write-Host "Starting Plex device sign-in…" -ForegroundColor Cyan
  $pin = Invoke-RestMethod -Method Post -Uri 'https://plex.tv/api/v2/pins.json?strong=true' -Headers $headers
  if (-not $pin -or -not $pin.id -or -not $pin.code) {
    throw "Unexpected response from Plex pins endpoint."
  }

  $code = [string]$pin.code
  $pinId = [string]$pin.id
  $linkUrl = "https://plex.tv/link?code=$code"

  Write-Host "Open this URL, sign in to Plex, and enter the code:" -ForegroundColor Yellow
  Write-Host "  $linkUrl"
  Write-Host "Code: $code" -ForegroundColor Yellow

  try {
    Start-Process $linkUrl | Out-Null
  } catch {
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    $poll = Invoke-RestMethod -Method Get -Uri ("https://plex.tv/api/v2/pins/$pinId.json?code=$code") -Headers $headers
    if ($poll -and $poll.authToken) {
      $authToken = [string]$poll.authToken

      # Exchange the auth token for a one-time claim token.
      $claimHeaders = @{} + $headers
      $claimHeaders['X-Plex-Token'] = $authToken
      $claim = Invoke-RestMethod -Method Get -Uri 'https://plex.tv/api/claim/token.json' -Headers $claimHeaders

      if ($claim -and $claim.token) {
        return [string]$claim.token
      }

      throw "Signed in to Plex, but failed to retrieve claim token."
    }
  }

  throw "Timed out waiting for Plex authorization."
}
...
if ([string]::IsNullOrWhiteSpace($plexClaim)) {
  $resp = Read-Host -Prompt "Auto-generate a Plex claim token by signing in (recommended)? (Y/n)"
  if ($resp -notin @('n','N','no','NO')) {
    try {
      $clientId = "eden-viewer-setup-$([Guid]::NewGuid().ToString('N'))"
      $plexClaim = Get-PlexClaimToken -ClientIdentifier $clientId
      Write-Host "Got Plex claim token." -ForegroundColor Green
    } catch {
      Write-Warning "Could not auto-generate Plex claim token. You can paste one manually later. Error: $($_.Exception.Message)"
      $plexClaim = ''
    }
  }
}
